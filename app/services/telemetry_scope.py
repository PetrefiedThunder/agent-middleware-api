...
class TelemetryScope:
    ...
    async def create_pipeline(
        ...
        logger.info(f"Created telemetry pipeline {pipeline_id} for {tenant_id}/{service_name}")
        return pipeline
    ...
    def _detect_anomalies_in_batch(
        ...
        if len(error_events) > len(events) * 0.3 and len(events) >= 5:
            anomaly = {
                ...
                "description": f"Error rate spike: {len(error_events)}/{len(events)} events are errors.",
                "severity": "high",
                "suggested_fix": "Check recent deployments. Review error logs for root cause.",
        ...
        if latencies:
            ...
            if avg_latency > 5000:
                anomaly = {
                    ...
                    "description": f"Average latency {avg_latency:.0f}ms exceeds 5000ms threshold.",
                    "severity": "medium",
                    "suggested_fix": "Profile slow endpoints. Check database queries and external API calls.",
                    "detected_at": datetime.now(timezone.utc).isoformat(),
                }
                anomalies.append(anomaly)
        ...
        for path, count in failed_endpoints.items():
            if count >= 3:
                anomaly = {
                    ...
                    "description": f"Endpoint {path} failed {count} times in this batch.",
                    "severity": "high",
                    "suggested_fix": f"Investigate {path}. Check handler logic and dependencies.",
                    "detected_at": datetime.now(timezone.utc).isoformat(),
                }
                anomalies.append(anomaly)
    ...
    async def generate_auto_pr(
        ...
        if not anomaly:
            raise ValueError(f"Anomaly {anomaly_id} not found in pipeline {pipeline_id}")

        pr = {
            "pr_id": f"pr-{uuid.uuid4().hex[:8]}",
            "pipeline_id": pipeline_id,
            "anomaly_id": anomaly_id,
            "title": f"[Auto-Fix] {anomaly['event_type']}: {anomaly['description'][:60]}",
            "body": (
                f"## Automated Fix\n\n"
                f"**Anomaly:** {anomaly['description']}\n"
                f"**Severity:** {anomaly['severity']}\n"
                f"**Suggested Fix:** {anomaly.get('suggested_fix', 'Review manually')}\n\n"
    ...
    async def list_pipelines(self, tenant_id: str | None = None) -> list[TelemetryPipeline]:
