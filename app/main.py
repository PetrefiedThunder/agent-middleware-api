...
app = FastAPI(
    ...
    description=(
        ...
        "This is a production-beta proof of an agent operations control plane, "
        "not complete production agent banking. The demonstrated trust artifacts "
        "are signed permits and signed receipts for governed MCP tool calls.\n\n"
        "### What The Proof Shows\n\n"
        "- **Discover** — autonomous clients start with `/.well-known/agent.json`, "
        "`/llm.txt`, `/mcp/tools.json`, and this OpenAPI contract\n"
        "- **Authorize** — signed permits bind wallet, key, tool, scope, budget, "
        "expiry, nonce, and revocation state\n"
        "- **Invoke and meter** — governed MCP invocation checks the permit before "
        "running the tool and charges the wallet once\n"
        "- **Receipt** — successful calls return signed receipts tied to ledger "
        "and audit records\n"
        "- **Audit and verify** — audit-chain verification gives operators tamper "
        "evidence for wallet operation history\n"
        "- **Replay and denial** — replay is safe and does not double charge; "
        "out-of-scope actions are denied and audited with denial receipts\n\n"
        "### Proof Surfaces\n\n"
        ...
        "### For Agents\n\n"
        "This API is designed for programmatic consumption. Before assuming real "
        "side effects, read `/health/dependencies` for simulation and dependency "
        "truth."
    ),
    docs_url="/docs",
    redoc_url="/redoc",
