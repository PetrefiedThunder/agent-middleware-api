...
def test_dispatch_call_slot_marker_backfills_false_and_downgrades(
    tmp_path,
    monkeypatch,
):
    """Legacy attempts must never release a slot they did not reserve."""
    db_path = tmp_path / "dispatch-call-slot-marker.db"
    async_url = f"sqlite+aiosqlite:///{db_path}"
    sync_url = f"sqlite:///{db_path}"
    monkeypatch.setenv("DATABASE_URL", async_url)
    config = Config("alembic.ini")
    command.upgrade(config, "037_merge_trust_heads")
    asyncio.set_event_loop(asyncio.new_event_loop())
        ...
        connection.execute(
            text(
                ...
                INSERT INTO mcp_dispatch_attempts (
                    ...
                    'dsp-pre-slot-marker', 'idm-pre-slot-marker',
                    'agt-pre-slot-marker', 'permit-pre-slot-marker',
                    'partner.legacy', 'partner_legacy',
    ...
    command.upgrade(config, "038_dispatch_call_slot_marker")
    engine = create_engine(sync_url)
    columns = {
        column["name"]: column
        for column in inspect(engine).get_columns("mcp_dispatch_attempts")
    }
    assert columns["call_slot_reserved"]["nullable"] is False
    with engine.begin() as connection:
        assert not bool(
            connection.execute(
                text(
                    """
                SELECT call_slot_reserved
                FROM mcp_dispatch_attempts
                WHERE attempt_id = 'dsp-pre-slot-marker'
                """
                )
            ).scalar_one()
        )
        # A rolling old worker that omits the additive column also gets the
        # conservative false default. A new worker may explicitly persist true.
        connection.execute(
            text(
                """
                INSERT INTO mcp_dispatch_attempts (
                    ...
                    'dsp-old-worker-slot', 'idm-old-worker-slot',
                    'agt-old-worker-slot', 'permit-old-worker-slot',
                    'partner.old', 'partner_old',
                    'https://partner.example', :request_hash,
                    1, 0, 'prepared'
                )
                """
        ...
        connection.execute(
            text(
                ...
                INSERT INTO mcp_dispatch_attempts (
                    attempt_id, idempotency_record_id, wallet_id, permit_id,
                    public_tool_id, upstream_tool_name, upstream_origin,
                    request_hash, credits_authorized, credits_charged,
                    call_slot_reserved, state
                ) VALUES (
                    'dsp-new-worker-slot', 'idm-new-worker-slot',
                    'agt-new-worker-slot', 'permit-new-worker-slot',
                    'partner.new', 'partner_new',
                    'https://partner.example', :request_hash,
                    1, 0, 1, 'prepared'
                )
                """
        ...
        markers = connection.execute(
            text(
                """
                SELECT attempt_id, call_slot_reserved
                FROM mcp_dispatch_attempts
                WHERE attempt_id LIKE 'dsp-%-worker-slot'
                ORDER BY attempt_id
                """
            )
        ).all()
    assert [(attempt_id, bool(marker)) for attempt_id, marker in markers] == [
        ("dsp-new-worker-slot", True),
        ("dsp-old-worker-slot", False),
    ]
    engine.dispose()

    command.downgrade(config, "037_merge_trust_heads")
    engine = create_engine(sync_url)
    assert "call_slot_reserved" not in {
        column["name"]
        for column in inspect(engine).get_columns("mcp_dispatch_attempts")
    }
    with engine.begin() as connection:
