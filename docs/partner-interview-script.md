...
2. **The product does not exist for the first 25 minutes.** If asked, say
   "we're researching how teams handle agent tool-call failures — I'll show
   you what we're building at the end if you want."
...
5. **Never supply the vocabulary.** Do not say idempotency, receipt,
   ...
   reach for — that's the landing-page copy.
...
> I'm not going to pitch you anything for the next half hour; I want to hear
...
Open with it. Everything else is follow-up.
...
If yes, excavate the story completely before moving on:
...
Whatever they built, take it seriously — it is the real competitor.

- "You added idempotency keys at the tool layer — how long did that take?
...
- "If the fix you built broke tomorrow, what would you do first: rebuild it,
  or look for a vendor?"
...
- The commitment test — ask for something that costs them:
  ...
  > Would you put an engineer on that for an afternoon?"

  An afternoon of a named engineer is the smallest real currency. "Sounds
  cool, keep me posted" is a no; write it down as a no.
...
| Field | Answer |
|---|---|
| Incident in past tense? (what, when, cost) | |
...
| Commitment test result (afternoon: yes/no) | |
| Disqualified? Why? | |
...
- **3+ of 5 interviews** surface a past-tense incident **and** at least
  **2 of 5** pass the commitment test → proceed to a paid-pilot conversation
  with the strongest one.
- Incidents everywhere but **zero** commitment → the problem is real and the
  build-it-yourself answer is winning; revisit whether the wedge is evidence
  (sell to whoever outside eng asks for the record) rather than reliability.
- **No past-tense incidents in 5 interviews** → stripe/ai#402 and the
  framework issues are ahead of the market; pause the sales motion, keep the
  gateway as the proof asset, and re-interview in a quarter.
