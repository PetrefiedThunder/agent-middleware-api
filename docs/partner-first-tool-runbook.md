...
15. **Partner verifies the receipt offline, in their own environment.** Export
    the portable bundle and public key set, then verify with no gateway access
    and no credentials:
    ...
    # The verifier is not on PyPI. Install the checked-out SDK or the wheel
    # attached to its release.
    pip install ./b2a_sdk
    python -m b2a_sdk.verify_cli --bundle receipt-bundle.json --keys trust-keys.json
    ```

    Expect `VERIFIED` and exit code 0; tampering with any field in
    `signing_input` must produce `INVALID`. The partner engineer must run this
    step on the partner's machine so verification does not trust the issuer at
    read time.
