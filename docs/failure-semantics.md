...
The stale window is never shorter than five minutes and expands to cover the
configured connection timeout, MCP initialization timeout, tool-call timeout,
and a bounded teardown margin. A valid long-running call therefore cannot be
declared stale before its configured execution deadline.
