...
Concurrent work later changed 18 paths in that accepted manifest. The frozen
packet remains valid for its exact bytes, but the current working tree requires
renewed source binding and affected-gate reruns before use in a pilot.
...
This research reran its pure models and a current source-hash audit, not the
application suites owned by the engineering program. All 572 whitelisted files
...
check found 18 changed accepted paths and zero missing files. The accepted
...
- **What passed:** bounded model/arithmetic checks; all 572 listed files still
  ...
  commands on that frozen source. The current checkout matches 554/572 and is
  not qualified by those runs.
