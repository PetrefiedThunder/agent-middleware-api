...
The current checkout no longer equals the accepted snapshot. A fresh completion
check at `2026-08-31T22:23:12Z` found **554/572** entries matching and 18
mismatches, including 10 of the 22 program-changed paths. Those edits appeared
after the final acceptance commands and have no replacement source manifest or
test packet cited here. The ten passing commands therefore apply to frozen
manifest `f0c7…`, not to every current working-tree byte. The mismatch inventory
is recorded under “Blockers that remain.”
...
| The local crash harness now joins a duplicate-visible upstream effect, response loss, charged uncertainty, worker death before receipt persistence, reconciliation, exact replay, and offline signature verification. The example deployment manifest was also corrected from migration 033 to the already-existing migration 034. | Frozen [`test_remote_response_loss_recovers_charged_receipt_verified_offline`](/tmp/amw-launch-20260831/acceptance-pg-crash/tests/test_mcp_postgres_multiprocess.py#L1317), frozen [`partner_write`](/tmp/amw-launch-20260831/acceptance-pg-crash/tests/support/mcp_remote_partner_app.py#L147), and [`railway-customer-manifest.example.json`](/Users/sellers/Documents/GitHub/agent-middleware-api/docs/railway-customer-manifest.example.json) | The nine-case PostgreSQL process suite passed under the final manifest. The upstream is an explicitly synthetic test fixture, so this closes a local harness gap only; it is not partner evidence. |
...
[`docs/aegis/work/2026-08-31-program-control/README.md`](/Users/sellers/Documents/GitHub/agent-middleware-api/docs/aegis/work/2026-08-31-program-control/README.md#L693).
...
6. The current checkout has 18 post-acceptance hash mismatches against `f0c7…`:
...
- Evidence action / check performed: recomputed SHA-256 for all 572 manifest
  entries against the current repository, checked current `HEAD`, read every
...
- Result / exit status: the frozen manifest and all ten log hashes verified, all
  ...
  `46d7310a3b771542dfb1fe874b5cff9d6bf137b2`. The fresh current-checkout
  comparison intentionally failed the exact-source assertion: 554/572 entries
  match and 18 differ, including 10/22 program-changed paths.
- Covered scope: frozen accepted source identity, program-selected code/test/doc
  delta, retained local runtime results, current byte-drift detection, and
...
- Residual risk: current working-tree edits are not source-bound to the accepted
  packet; external effect semantics, ingress limits, key timing, and demand
  remain bounded or unverified.
- Confidence grade: **B** for the frozen local integrated delta, **C** for the
  current working tree and lead status; no confidence grade is assigned to
  unperformed partner or commercial outcomes.
