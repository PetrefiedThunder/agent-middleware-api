...
**Current finding: local technical gates G0–G5 are accepted for the frozen
captured source and environments. The current working tree is not qualified:
concurrent work changed 18 accepted manifest paths after that cutoff. G6, the
partner-owned experiment, is not accepted.**
...
current checkout and four isolated tested roots. All five comparisons matched
when [technical-evidence.json](technical-evidence.json) was generated. A later
fresh audit found 18 mismatches and zero missing files. The frozen packet remains
valid for its exact bytes; the current tree needs a new manifest and affected-
gate reruns, or the pilot must use the frozen accepted archive.
...
match. The manifest is a scoped inventory, not a signed attestation or a freeze
of unlisted files.
...
- What passed: all 572 listed files still match each of the four frozen tested
  ...
  the frozen packet is accepted. The current checkout matches 554/572.
