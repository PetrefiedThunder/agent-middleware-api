...
> **Status note (updated 2026-08-27):** this report is a dated snapshot. Since
> it ran, the Railway-generated origin above stopped being publicly routable (it
> now returns 404); the canonical public API base is
> `https://api.thisisatest.tech/`. The GitHub repository was private as of the
> original 2026-08-17 status note but is now **public**, so anonymous
> `github.com` fetches no longer require operator-granted source access.
> Reproduction commands still use `API=https://api.thisisatest.tech`.
